...
class RobustAE():
    def __init__(self,hidden_size,visible_size,sparse_ratio=0.1, random_state = 0):
    ...
    def one_norm_derivative(self, x):
        return
    def split(self,x):
        sparse = x * self.mask
    ...
    def cost(self , data , lambda_):
        ...
        grad = [Wgrad , b1grad , b2grad , maskgrad]
    ...
    def fit(self, data, lambda_, learning_rate=0.1, iteration = 25):
        ...
        self.mask = rng.normal(loc = 1., size = (data.shape[0], data.shape[1]))
        print "training..."
        print
        training_cost = []
...
if __name__== '__main__':
    ...
    rae.fit(data,lambda_=0.5,learning_rate=0.1, iteration=2)
    rae.transform(data)
